...
#endif
  "<br/><form action='st' method='get'><button>" D_CMND_STATUS "</button></form>"
...
/*** WatchDog adder ***/
extern String expiretimes[];
extern int indexTimes;
#define EXPIRETIMES 5

const char S_STATUS[] PROGMEM = "Status";
const char HTTP_BTN_MENU5[] PROGMEM =
  "<br/><form action='st' method='get'><button>Refresh</button></form>";
...
void HandleStatus ()
{
    ...
    AddLog_P(LOG_LEVEL_DEBUG, S_LOG_HTTP, S_STATUS);
    
    String page = FPSTR(HTTP_HEAD);
    page.replace(F("{v}"), F("Status"));
    page += F("<div style='text-align:center;'><b>Recent Resets</b><br/><br/>");
    page += F("Current Time:"); page += GetDateAndTime(); page += F("<br/><br/>");
    page += F("<style>td{padding:0px 5px;}table, th, td {border: 1px solid black;}table {border-collapse: collapse;}</style>");
    page += F("<table style='width:100%;align:center;'>");
    for ( int i = 0; i < EXPIRETIMES; i++ ) {
        page += F("<tr><th>"); page += (indexTimes == 0 || i != indexTimes-1) ? "&nbsp;&nbsp; " : "*";
        page += F("</th><td>"); page += expireTimes[i].c_str();
        page += F("</td></tr>");
    }
    page += F("</table></div>");
